    ...
    print = function() {
      ...
      if (self$i == 0) {
        ...
        if (length(self$dotdotdot) != 0) {
          dots <- paste0(
            names(self$dotdotdot),
            "=self$dotdotdot$",
            names(self$dotdotdot),
            collapse = ", "
          )
          eval(parse(
            text = paste0("eta <- self$fn(dtime, self$i, self$n,", dots, ")")
          ))
        } else {
      ...
      width <- getOption("width") -
        nchar("|100.0% ~elapsed: 99 h 00 m 00 s") -
        2
...
#' }
#' @export
lazyProgressBar <- function(n, method = "average", fn = NULL, ...) {
...
.drift_fn <- function(dtime, i, n) {
  left <- n - i
  eta <- sum(
    ((dtime[[i]] - dtime[[1]]) / ifelse((te <- i - 1) == 0, 1, te)) *
      seq(1, left)
  ) +
    left * dtime[[i]]
  if (eta < 0) {
